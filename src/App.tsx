...
const loadProvinces = async () => {
  try {
    const options: Array<Option> = await axios.get(
      'http://danube-topcars.stable.dasouche.net/dictionary/dictionaryAction/province.json',
    )
    const sortedOptions = options.sort((a, b) =>
      a.letter.localeCompare(b.letter),
    )
    const groupedOptions: Array<{ key: string; row: Array<Option> }> =
      Object.values(
        sortedOptions.reduce(
          (a: { [key: string]: { key: string; row: Array<Option> } }, c) => {
            if (!a[c.letter]) {
...
function App() {
  const loadFns = [
    loadProvinces,
    loadCitiesByProvinceCode,
    loadRegionByCityCode,
  ]
